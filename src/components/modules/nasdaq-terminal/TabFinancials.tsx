        ...
        <div className="flex items-center gap-1 bg-white/[0.03] rounded-lg p-0.5">
          <button
            ...
            className={`px-2 py-1 rounded text-[10px] font-medium ${
              period === 'annual' ? 'bg-white/10 text-white' : 'text-white/40'
            }`}
          >
            Yıllık
          </button>
          <button
            ...
            className={`px-2 py-1 rounded text-[10px] font-medium ${
              period === 'quarter' ? 'bg-white/10 text-white' : 'text-white/40'
            }`}
          >
            Çeyreklik
          </button>
    ...
    <table className="w-full">
      <thead>
        <tr className="border-b border-white/5">
          <th className="text-left px-2 sm:px-4 py-2 sm:py-2.5 text-[10px] text-white/30 uppercase tracking-wider font-medium w-32 sm:w-40">
            Kalem
          </th>
          {items.map((item, i) => (
            <th key={i} className="text-right px-2 sm:px-3 py-2 sm:py-2.5 text-[10px] text-white/40 font-medium">
              {item.date?.split('-')[0]}
            </th>
          ))}
        </tr>
      ...
      <tbody>
        {rows.map((row, ri) => (
          <tr key={ri} className="border-b border-white/[0.03] hover:bg-white/[0.02]">
            <td className="px-2 sm:px-4 py-1.5 sm:py-2 text-xs text-white/50">{row.label}</td>
            {items.map((item, ci) => {
...
function EmptyTable() {
  return <div className="p-4 sm:p-8 text-center text-white/30 text-sm">Finansal veri bulunamadi</div>
}
    ...
    <div className="relative min-h-[40vh] flex flex-col items-center justify-center overflow-hidden">
      <div className="absolute inset-0 data-stream pointer-events-none" />
        ...
        <div className="text-center">
          ...
          <p className="text-[9px] text-white/20 mt-0.5">Gelir, bilanco, nakit akis</p>
        </div>
