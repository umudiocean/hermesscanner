...
const SEGMENT_CONFIG: { key: SegmentFilter; label: string; desc: string; icon: React.ReactNode; color: string }[] = [
  ...
  { key: 'MEGA', label: 'MEGA', desc: '$200B+', icon: <Crown size={13} />, color: 'text-gold-400 bg-gold-500/8 border-amber-500/20 hover:border-stroke-gold-strong' },
  { key: 'LARGE', label: 'LARGE', desc: '$10B-$200B', icon: <Building2 size={13} />, color: 'text-info-400 bg-info-400/8 border-info-400/30 hover:border-violet-500/40' },
  { key: 'MID', label: 'MID', desc: '$2B-$10B', icon: <Store size={13} />, color: 'text-info-400 bg-info-400/8 border-info-400/30 hover:border-blue-500/40' },
  ...
  { key: 'MICRO', label: 'MICRO', desc: '<$300M', icon: <Bug size={13} />, color: 'text-warning-400 bg-orange-500/8 border-orange-500/20 hover:border-orange-500/40' },
]

const SIGNAL_CONFIG: Record<string, { color: string; icon: React.ReactNode; label: string }> = {
  STRONG: { color: 'text-gold-400 bg-gold-500/12 border-amber-500/25', icon: <Zap size={12} />, label: 'Guclu' },
  GOOD: { color: 'text-success-400 bg-success-400/12 border-success-400/25', icon: <TrendingUp size={12} />, label: 'Iyi' },
  NEUTRAL: { color: 'text-text-secondary bg-surface-3 border-stroke', icon: <Minus size={12} />, label: 'Notr' },
  WEAK: { color: 'text-warning-400 bg-orange-500/12 border-orange-500/25', icon: <AlertTriangle size={12} />, label: 'Zayif' },
  BAD: { color: 'text-danger-400 bg-danger-400/12 border-danger-400/30', icon: <TrendingDown size={12} />, label: 'Kotu' },
}
...
export default function TabStocks({ onSelectSymbol }: TabStocksProps) {
  ...
  return (
    <div className="space-y-2 sm:space-y-3 px-2 sm:px-4 lg:px-6 animate-fade-in">
      ...
      <div className="flex items-center gap-1.5 flex-wrap">
        ...
        <button
          ...
          className={`relative flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl border text-xs font-bold transition-all duration-300 overflow-hidden
            ${watchlistOnly
              ? 'text-gold-300 bg-gradient-to-r from-amber-500/15 to-amber-600/10 border-amber-400/40 ring-1 ring-amber-400/20 shadow-lg shadow-amber-500/25'
              : 'text-gold-400/40 bg-surface-2 border-stroke-subtle hover:border-amber-400/25 hover:text-gold-400/70 hover:bg-gold-500/[0.04]'
            }`}
        >
          {watchlistOnly && (
            <>
              <span className="absolute inset-0 rounded-xl bg-gold-400/[0.07] animate-pulse pointer-events-none" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
        ...
        <div className="bg-surface-3 rounded-xl border border-stroke-subtle p-2.5">
          ...
          <div className="flex gap-1 flex-wrap">
            {[
              ...
              { key: 'medium' as const, label: '%50-69', color: 'text-gold-400 bg-gold-500/10 border-amber-500/25' },
              { key: 'low' as const, label: '<%50', color: 'text-danger-400 bg-danger-400/10 border-danger-400/30' },
            ].map(f => (
        ...
        <div className="bg-surface-3 rounded-xl border border-stroke-subtle p-2.5">
          ...
          <div className="flex gap-1 flex-wrap">
            {[
              ...
              { key: 'NORMAL', label: 'Normal', color: 'text-text-secondary bg-surface-3 border-stroke' },
              { key: 'PAHALI', label: 'Pahali', color: 'text-warning-400 bg-orange-500/10 border-orange-500/25' },
              { key: 'COK PAHALI', label: 'Cok Pahali', color: 'text-danger-400 bg-danger-400/10 border-danger-400/30' },
            ].map(f => (
        ...
        <div className="bg-surface-3 rounded-xl border border-stroke-subtle p-2.5">
          ...
          <div className="flex gap-1 flex-wrap">
            {[
              { key: 'all', label: 'Tumu', color: 'text-text-secondary bg-surface-3 border-stroke' },
              { key: 'STRONG', label: 'Guclu', color: 'text-gold-400 bg-gold-500/10 border-amber-500/25' },
              { key: 'GOOD', label: 'Iyi', color: 'text-success-400 bg-success-400/10 border-success-400/25' },
              { key: 'NEUTRAL', label: 'Notr', color: 'text-text-secondary bg-surface-3 border-stroke' },
              { key: 'WEAK', label: 'Zayif', color: 'text-warning-400 bg-orange-500/10 border-orange-500/25' },
              { key: 'BAD', label: 'Kotu', color: 'text-danger-400 bg-danger-400/10 border-danger-400/30' },
            ].map(f => (
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center gap-2 sm:gap-2.5 justify-between">
        <div className="flex items-center gap-2 flex-wrap">
          <div className="relative group">
            <Search size={15} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-text-quaternary group-focus-within:text-info-400 transition-colors" />
            <input type="text" value={search}
              ...
              className="pl-8 pr-3 py-1.5 rounded-xl bg-surface-3 border border-stroke text-sm text-white
                         placeholder-white/25 focus:outline-none focus:border-info-400/30 focus:bg-surface-3 w-full sm:w-48 transition-all duration-200"
            />
          </div>
          <div className="relative">
            ...
            <select value={sectorFilter}
              ...
              className="pl-7 pr-3 py-1.5 rounded-xl bg-surface-3 border border-stroke text-sm text-white
                         focus:outline-none focus:border-info-400/30 cursor-pointer appearance-none transition-all duration-200">
      ...
      {/* Tooltip */}
      {tooltip && (
        <div className="flex items-center gap-1.5 px-3 py-1.5 bg-info-400/8 border border-violet-500/15 rounded-xl text-xs text-violet-200 animate-fade-in">
          <Info size={13} className="text-info-400 shrink-0" />
          {tooltip}
        </div>
      )}
                  ...
                  <tr key={s.symbol}
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        {indexMap[s.symbol]?.map(idx => (
                          <span key={idx} className={`text-[8px] font-bold px-1 py-0 rounded ${
                            idx === 'SP500' ? 'text-info-400/70 bg-info-400/10' :
                            idx === 'NDX100' ? 'text-info-400/70 bg-info-400/10' :
                            idx === 'DJIA' ? 'text-gold-300/70 bg-gold-500/10' : 'text-text-tertiary bg-surface-3'
                          }`}>{idx}</span>
                        ))}
                      </div>
                    </td>
                    ...
                    <td className="px-1 py-2 text-right">
                      <div className="group/score relative flex items-center justify-end cursor-pointer">
                        ...
                        <div className="hidden group-hover/score:block absolute z-50 bottom-full right-0 mb-2 w-52 bg-[#141420] border border-stroke rounded-xl shadow-2xl p-3">
                          ...
                          {s.gated && (
                            <div className="mt-2 text-[9px] text-danger-400/80 flex items-center gap-1">
                    ...
                    <td className="px-1 py-2 text-right">
                      <span className={`text-[11px] tabular-nums font-medium ${
                        (s.confidence || 0) >= 70 ? 'text-success-400/60' : (s.confidence || 0) >= 50 ? 'text-gold-400/60' : 'text-text-quaternary'
                    ...
                    <td className="px-1 py-2 text-right">
                      <span className={`inline-flex items-center gap-0.5 text-[13px] tabular-nums font-semibold ${s.changePercent >= 0 ? 'text-success-400' : 'text-danger-400'}`}>
                    ...
                    <td className="px-1 py-2 text-right">
                      <span className={`text-[11px] tabular-nums font-medium ${
                        (s.altmanZ || 0) >= 3 ? 'text-success-400/70' : (s.altmanZ || 0) >= 1.8 ? 'text-gold-400/70' : (s.altmanZ || 0) > 0 ? 'text-danger-400/70' : 'text-text-tertiary'
                      }`}>{s.altmanZ > 0 ? s.altmanZ.toFixed(1) : '\u2014'}</span>
                    </td>
                    <td className="px-1 py-2 text-right">
                      <span className={`text-[11px] tabular-nums font-bold ${
                        (s.piotroski || 0) >= 7 ? 'text-success-400' : (s.piotroski || 0) >= 5 ? 'text-gold-400/70' : (s.piotroski || 0) > 0 ? 'text-warning-400/70' : 'text-text-tertiary'
                      }`}>{s.piotroski > 0 ? s.piotroski : '\u2014'}</span>
                    </td>
                    <td className="px-1 py-2 text-right">
                      <span className={`text-[11px] tabular-nums font-medium ${
                        (s.dcfUpside || 0) > 20 ? 'text-success-400' : (s.dcfUpside || 0) > 0 ? 'text-success-400/60' : (s.dcfUpside || 0) < -20 ? 'text-danger-400' : (s.dcfUpside || 0) < 0 ? 'text-danger-400/60' : 'text-text-tertiary'
                      }`}>{s.dcfUpside ? `${s.dcfUpside > 0 ? '+' : ''}${s.dcfUpside.toFixed(0)}%` : '\u2014'}</span>
                    </td>
                    <td className="px-1 py-2 text-right">
                      <span className={`text-[10px] tabular-nums ${
                        (s.analystEpsRevision30d || 0) > 0 ? 'text-success-400/80' : (s.analystEpsRevision30d || 0) < 0 ? 'text-danger-400/80' : 'text-text-quaternary'
                      }`}>
                    ...
                    <td className="px-1 py-2 text-right">
                      <span className={`text-[10px] tabular-nums ${
                        (s.analystEpsRevision90d || 0) > 0 ? 'text-success-400/70' : (s.analystEpsRevision90d || 0) < 0 ? 'text-danger-400/70' : 'text-text-quaternary'
                      }`}>
                        {(s.analystEpsRevision90d || 0) !== 0 ? `${(s.analystEpsRevision90d || 0) > 0 ? '+' : ''}${(s.analystEpsRevision90d || 0).toFixed(1)}%` : '\u2014'}
                      </span>
                    </td>
                    <td className="px-1 py-2 text-right">
                      <span className={`text-[13px] tabular-nums ${s.roe > 0 ? 'text-success-400/70' : s.roe < 0 ? 'text-danger-400/70' : 'text-text-quaternary'}`}>
                        {fmtPct(s.roe)}
                      </span>
                    </td>
                    <td className="px-1 py-2 text-right">
                      <span className={`text-[13px] tabular-nums ${s.debtEquity > 2 ? 'text-danger-400/70' : 'text-text-tertiary'}`}>{fmtR(s.debtEquity)}</span>
                    </td>
                    <td className="px-1 py-2 text-right">
                      <span className={`text-[11px] tabular-nums font-semibold px-1.5 py-0.5 rounded-full ${
                        ...
                        (s.riskScore || 50) <= 50 ? 'text-gold-400 bg-gold-500/10' :
                        (s.riskScore || 50) <= 75 ? 'text-warning-400 bg-orange-500/10' :
                    ...
                    <td className="px-1 py-2 text-right">
                      <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full whitespace-nowrap ${
                        ...
                        s.valuationLabel === 'NORMAL' ? 'text-text-secondary bg-surface-3' :
                        s.valuationLabel === 'PAHALI' ? 'text-warning-400 bg-orange-500/10' :
                        s.valuationLabel === 'COK PAHALI' ? 'text-danger-400 bg-danger-400/10' :
                    ...
                    <td className="px-1 py-2 text-right">
                      {(s.overvalScore || 0) > 0 ? (
                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full whitespace-nowrap ${
                          ...
                          s.overvalLevel === 'HIGH' ? 'text-danger-400 bg-danger-400/15' :
                          s.overvalLevel === 'MEDIUM' ? 'text-warning-400 bg-orange-500/10' :
                        ...
                        <div className="flex flex-wrap gap-0.5 mt-0.5 justify-end">
                          {s.badges.slice(0, 2).map((b, bi) => (
                            <span key={bi} className={`text-[8px] px-1 py-0 rounded ${
                              ...
                              b.severity === 'negative' ? 'text-danger-400 bg-danger-400/10' :
                              b.severity === 'warning' ? 'text-gold-400 bg-gold-500/10' :
                              'text-info-400 bg-info-400/10'
                      ...
                      {s.priceTarget > 0 ? (
                        <div className="flex flex-col items-end">
                          <span className={`font-mono text-[11px] font-semibold ${
                            s.priceTarget > s.price ? 'text-success-400' : s.priceTarget < s.price ? 'text-danger-400' : 'text-text-secondary'
                    ...
                    <td className="px-1 py-2 text-right">
                      {s.yearLow > 0 ? (
                        <div className="flex flex-col items-end">
                          <span className="font-mono text-[11px] text-danger-400/70">${s.yearLow.toFixed(2)}</span>
                      ...
                      {(() => {
                        ...
                        return (
                          <span className={`font-mono text-[11px] font-bold ${
                            rr >= 2 ? 'text-success-400' : rr >= 1 ? 'text-gold-300' : 'text-danger-400'
                          }`}>{rr.toFixed(1)}</span>
                        )
                      })()}
                    </td>
                    <td className="px-1 py-2 text-right">
                      <span className={`text-[11px] tabular-nums ${
                        ...
                        (s.shortFloat || 0) > 0 ? 'text-warning-400/70' : 'text-text-quaternary'
...
function ThC({ field, label, sort, dir, onSort, w = '', align = 'left', tip, onTip }: {
  ...
  return (
    <th onClick={() => onSort(field)}
      ...
      {label}
      {sort === field
        ? <span className="text-info-400 ml-0.5 text-[10px]">{'\u25B2'}{dir === 'asc' ? '' : '\u25BC'}</span>
          ...
          <div className="flex ticker-scroll whitespace-nowrap">
            {[...TICKERS, ...TICKERS].map((t, i) => (
              <span key={i} className="inline-flex items-center gap-1 px-2 text-[9px] font-mono text-text-quaternary">
                {t}
                <span className={i % 3 === 0 ? 'text-success-400/20' : i % 3 === 1 ? 'text-danger-400/20' : 'text-text-quaternary'}>
                  {i % 3 === 0 ? '▲' : i % 3 === 1 ? '▼' : '—'}
                </span>
              </span>
            ))}
