  ...
  if (!symbol) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[40vh]">
        ...
        <p className="text-text-tertiary text-sm">Analist verilerini görmek için hisse seçin</p>
      </div>
    )
  }
        ...
        <div className="absolute inset-0 data-stream pointer-events-none" />
          ...
          <div className="relative w-14 h-14">
            <svg className="w-14 h-14" viewBox="0 0 100 100" style={{ animation: 'ring-spin 2s linear infinite' }}>
              ...
              <defs><linearGradient id="analGold" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#DCC273" /><stop offset="100%" stopColor="#8E7536" /></linearGradient></defs>
            </svg>
            <div className="absolute inset-0 flex items-center justify-center text-gold-400/80 text-lg">🎯</div>
          </div>
          <div className="text-center">
            <p className="text-xs font-semibold text-text-secondary">Analist tahminleri</p>
            <p className="text-[9px] text-text-tertiary mt-0.5">Konsensus, fiyat hedefleri</p>
          </div>
    ...
    <div className="space-y-2 sm:space-y-4">
      ...
      {priceTarget && (
        <div className="bg-[#0F0F15] rounded-xl border border-white/5 p-3 sm:p-4">
          <h4 className="text-[10px] text-text-tertiary uppercase tracking-wider mb-2 sm:mb-3">Fiyat Hedefi</h4>
      ...
      {surprises.length > 0 && (
        <div className="bg-[#0F0F15] rounded-xl border border-white/5 p-3 sm:p-4">
          <h4 className="text-[10px] text-text-tertiary uppercase tracking-wider mb-2 sm:mb-3">
        ...
        <div className="bg-[#0F0F15] rounded-xl border border-white/5 p-3 sm:p-4">
          ...
          <div className="space-y-1.5 max-h-64 overflow-y-auto">
            {grades.slice(0, 20).map((g, i) => (
              <div key={i} className="flex items-center gap-3 py-1.5 px-2 rounded hover:bg-surface-2">
                <span className="text-[10px] text-text-tertiary w-20 shrink-0">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/5">
                  <th className="text-left px-3 py-2 text-[10px] text-text-tertiary">Dönem</th>
                  <th className="text-right px-3 py-2 text-[10px] text-text-tertiary">EPS Tahmin</th>
                  <th className="text-right px-3 py-2 text-[10px] text-text-tertiary">Gelir Tahmin</th>
                  <th className="text-right px-3 py-2 text-[10px] text-text-tertiary">Analist #</th>
                </tr>
              </thead>
              <tbody>
                {estimates.slice(0, 6).map((e, i) => (
                  <tr key={i} className="border-b border-white/[0.03] hover:bg-surface-2">
                    <td className="px-3 py-1.5 text-xs text-text-secondary">{e.date}</td>
                    ...
                    <td className="px-3 py-1.5 text-[10px] text-text-tertiary text-right">
