...
export default function TabTreasury() {
  ...
  if (error) return (
    <div className="flex flex-col items-center justify-center min-h-[30vh] text-center">
      ...
      <button onClick={() => window.location.reload()} className="px-4 py-2 rounded-xl bg-white/[0.04] text-white/60 text-xs hover:bg-white/[0.08] transition-all">Yenile</button>
    </div>
  )
    ...
    <div className="space-y-2 sm:space-y-4 animate-fade-in">
      ...
      {data && (
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2 sm:gap-3">
          <div className="bg-[#151520] rounded-xl border border-white/[0.06] p-3 hover:border-amber-500/20 hover:shadow-md hover:shadow-amber-500/5 transition-all duration-300">
            <span className="text-[10px] text-white/40 uppercase">Toplam Holding</span>
          ...
          <div className="bg-[#151520] rounded-xl border border-white/[0.06] p-3 hover:border-white/[0.12] hover:shadow-md hover:shadow-black/20 transition-all duration-300">
            <span className="text-[10px] text-white/40 uppercase">Toplam Deger</span>
            <div className="text-base font-bold text-white tabular-nums mt-0.5">{formatLarge(data.total_value_usd)}</div>
          </div>
          <div className="bg-[#151520] rounded-xl border border-white/[0.06] p-3 hover:border-white/[0.12] hover:shadow-md hover:shadow-black/20 transition-all duration-300">
            <span className="text-[10px] text-white/40 uppercase">MCap Hakimiyeti</span>
      ...
      {data?.companies && (
        <div className="bg-[#151520] rounded-2xl border border-white/[0.06] p-3 sm:p-4 hover:border-white/[0.12] transition-all duration-300">
          <h4 className="text-xs font-bold text-white/50 uppercase mb-2 sm:mb-3">Sirketler</h4>

          {/* Mobile */}
          <div className="sm:hidden space-y-2">
            {data.companies.map((c) => (
              <div key={`${c.name}-${c.symbol}`} className="p-2 rounded-lg bg-white/[0.02] border border-white/[0.04]">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-bold text-white">{c.name} <span className="text-white/40">({c.symbol})</span></span>
                  <span className="text-[10px] text-white/40">{c.country}</span>
                </div>
                <div className="flex items-center justify-between text-[10px]">
                  <span className="text-white/50">{c.total_holdings?.toLocaleString()} {activeAsset.toUpperCase()}</span>
                  <span className="text-white/50">{formatLarge(c.total_current_value_usd)}</span>
            ...
            <table className="w-full text-left">
              ...
              <tbody>
                {data.companies.map((c, i) => {
                  ...
                  return (
                    <tr key={`${c.name}-${c.symbol}`} className="border-b border-white/[0.03] hover:bg-white/[0.02]">
                      <td className="px-2 py-2 text-xs text-white/40">{i + 1}</td>
                      <td className="px-2 py-2">
                        <span className="text-xs font-bold text-white">{c.name}</span>
                        <span className="text-[10px] text-white/40 ml-1">({c.symbol})</span>
                      </td>
                      <td className="px-2 py-2 text-right text-xs text-white/60 tabular-nums">{c.total_holdings?.toLocaleString()}</td>
                      <td className="px-2 py-2 text-right text-xs text-white/50 tabular-nums">{formatLarge(c.total_entry_value_usd)}</td>
                      ...
                      <td className="px-2 py-2 text-right text-xs text-white/40 tabular-nums">{c.percentage_of_total_supply?.toFixed(3)}%</td>
                    </tr>
                  )
                })}
              </tbody>
