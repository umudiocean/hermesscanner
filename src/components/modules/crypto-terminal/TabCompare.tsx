        ...
        <div className="absolute top-full left-0 mt-1 w-80 max-h-64 overflow-y-auto bg-surface-3 border border-stroke rounded-xl shadow-2xl shadow-black/40 z-50">
          {suggestions.map((c, i) => (
            ...
            >
              ...
              <span className="text-[11px] text-text-tertiary truncate">{c.name}</span>
            </button>
          ))}
        </div>
      )}
  ...
  if (coinIds.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] text-center">
        <GitCompare size={48} className="text-text-quaternary mb-4" />
        <h3 className="text-base sm:text-lg font-semibold text-white mb-2">Coin Karsilastir</h3>
        <p className="text-text-tertiary text-sm mb-6">Coin ismi veya sembol yazin (max 4 coin)</p>
            ...
            <div key={id} className="bg-surface-3 rounded-2xl border border-stroke-subtle p-3 sm:p-4 relative hover:border-stroke hover:shadow-lg hover:shadow-black/20 transition-all duration-300">
              ...
              <div className="flex items-center gap-2 mb-2 sm:mb-3">
                ...
                <div>
                  ...
                  <div className="text-[10px] text-text-tertiary">{detail.symbol?.toUpperCase()} #{md?.market_cap_rank}</div>
                </div>
              </div>
              ...
              {score && (
                <div className="mt-3 pt-3 border-t border-stroke-subtle">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[10px] text-text-tertiary">HERMES AI Skor</span>
                    ...
                    {CRYPTO_CATEGORY_KEYS
                      ...
                      return (
                        <div key={key} className="flex items-center gap-1">
                          ...
                          <span className="w-5 text-[8px] text-right text-text-tertiary">{Math.round(val)}</span>
                        </div>
                      )
                    })}
                  </div>
