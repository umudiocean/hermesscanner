    ...
    <div className="max-w-[1920px] mx-auto px-3 sm:px-4 lg:px-6 py-3 sm:py-4 animate-fade-in">
      ...
      <div className="flex items-center justify-between mb-2 sm:mb-4">
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-text-tertiary">{watchlistIds.length} coin takip ediliyor</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <div ref={searchRef} className="relative">
            ...
            {showSearch && (
              <div className="absolute right-0 top-full mt-1 w-72 bg-surface-3 border border-stroke rounded-xl shadow-2xl shadow-black/50 z-50 overflow-hidden">
                ...
                <div className="max-h-64 overflow-y-auto">
                  {searchLoading && <div className="p-3 text-center text-[10px] text-text-tertiary">Araniyor...</div>}
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center gap-2.5 px-3 py-2 text-left transition-colors ${
                          alreadyAdded ? 'opacity-40 cursor-not-allowed' : 'hover:bg-surface-3 cursor-pointer'
                        }`}
                      >
                        <div className="w-5 h-5 rounded-full bg-white/5 flex items-center justify-center flex-shrink-0 text-[8px] font-bold text-text-tertiary uppercase">
                          {r.symbol?.slice(0, 2)}
                        </div>
      ...
      {watchlistIds.length === 0 && !loading && (
        <div className="flex flex-col items-center justify-center py-12 sm:py-20 text-center">
          <Star size={40} className="text-text-quaternary mb-2 sm:mb-4" />
      ...
      {loading && (
        <div className="space-y-2">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="bg-surface-3 rounded-xl border border-stroke-subtle p-3 sm:p-4 h-14 animate-pulse" />
          ))}
        </div>
      )}
          ...
          <table className="w-full text-left border-collapse">
            <thead className="sticky top-0 bg-surface-1 z-10">
              <tr>
                ...
                <th className="py-2 px-2 text-[10px] font-bold text-text-tertiary uppercase text-right hidden xl:table-cell" title="Hedef Fiyat">Hedef</th>
                <th className="py-2 px-2 text-[10px] font-bold text-text-tertiary uppercase text-right hidden xl:table-cell" title="Dip Fiyat">Dip</th>
                <th className="py-2 px-2 text-[10px] font-bold text-text-tertiary uppercase text-center hidden xl:table-cell" title="Risk/Odul Orani">R:R</th>
                <WTH field="marketCap" align="right">MCap</WTH>
                <th className="py-2 px-2 text-[10px] font-bold text-text-tertiary uppercase text-center">Uyarilar</th>
              </tr>
            </thead>
            <tbody>
              {sortedCoins.map(coin => {
                ...
                return (
                  <tr key={coin.id} className="border-b border-white/[0.03] hover:bg-amber-500/[0.03] transition-colors group">
                      ...
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-1">
                            ...
                            <span className="text-[10px] text-text-tertiary hidden lg:inline truncate">{coin.name}</span>
                          </div>
                          {tags.length > 0 && (
                    ...
                    <td className="px-2 py-2.5 text-right text-[11px] text-text-secondary tabular-nums">{formatMcap(coin.market_cap)}</td>
                    <td className="px-2 py-2.5 text-center">
